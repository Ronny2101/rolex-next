...
const AddProperty = ({ initialValues, ...props }: any) => {
	...
	useEffect(() => {
		setInsertPropertyData({
			...
			propertyLimitedEdition: getPropertyData?.getProperty ? getPropertyData?.getProperty?.propertyLimitedEdition : false,
			propertyMaterial: getPropertyData?.getProperty ? getPropertyData?.getProperty?.propertyMaterial : '',
			propertyMovement: getPropertyData?.getProperty ? getPropertyData?.getProperty?.propertyMovement : '',
	...
	const doDisabledCheck = () => {
		if (
			...
			insertPropertyData.propertyLimitedEdition === '' || // @ts-ignore
			insertPropertyData.propertyMaterial === '' || // @ts-ignore
			insertPropertyData.propertyMovement === '' ||
					...
					<Stack className="config">
						<Stack className="description-box">
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									<Typography className="title">LimitedEdition</Typography>
									<select
										className={'select-description'}
										value={insertPropertyData.propertyLimitedEdition ? 'yes' : 'no'}
										defaultValue={insertPropertyData.propertyLimitedEdition ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertPropertyData({ ...insertPropertyData, propertyLimitedEdition: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									...
									<select
										...
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertPropertyData({ ...insertPropertyData, propertyMovement: value })
										}
									>
										<option selected={true} disabled={true} value={'select'}>
											Select
										</option>
										{propertyMovement.map((movement: any) => (
